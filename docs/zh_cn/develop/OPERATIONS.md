...
3. 模板命中右上角 X → 点它
4. 模板命中左上返回箭头 → 点它
5. **模板都没命中**：按轮次轮换三种兜底 —— 右上 X 固定坐标 → 左上返回固定坐标 → 底部「确定」OCR

> 第 3~5 步为什么需要三层：各界面按钮**样式不同**，且同一模板在不同帧上分数能从 0.4 跳到 1.0
> （实测 `叉1.png` 在「落潮驿站」弹窗上有 1.000 也有 0.415，取决于截图落在动画哪一帧）。
> 任何单一策略都会有失效的界面 —— 实测该弹窗上两个模板同时失配，最后靠轮换点到底部「确定」才出来。
